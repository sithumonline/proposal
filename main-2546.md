...
Social media, which is used by most individuals, has become an integral part of modern lifestyle. However, these social networks use centralized servers to store user data, which is an approach shown to have many disadvantages such as users have no control over their data and users have no control over the platform.
